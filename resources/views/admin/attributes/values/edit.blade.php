        ...
        <div class="p-6">
            <form action="{{ route('admin.attributes.values.update', [$attribute->id, $value->id]) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        @if($attribute->type == 'color')
                            <div class="flex space-x-3">
                                <input type="color" name="color_picker" id="color_picker" class="h-10 w-10 rounded border border-gray-300 dark:border-gray-700 cursor-pointer" value="{{ old('value', $value->value) }}">
                                <input type="text" name="value" id="value" class="form-input-admin @error('value') border-red-500 @enderror" value="{{ old('value', $value->value) }}" required>
                            </div>
                        @else
                            <input type="text" name="value" id="value" class="form-input-admin @error('value') border-red-500 @enderror" value="{{ old('value', $value->value) }}" required>
                        @endif
                    ...
                    <div class="pt-4">
                        <button type="submit" class="btn-primary-admin">Сохранить изменения</button>
                    </div>
                </div>
            </form>
        </div>
